...
-- Adiciona rastreabilidade de fechamento no imóvel
ALTER TABLE public.imoveis
  ADD COLUMN IF NOT EXISTS fechado_em timestamptz,
  ADD COLUMN IF NOT EXISTS fechado_por uuid REFERENCES public.profiles(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS lead_fechamento_id uuid REFERENCES public.vendas_leads(id) ON DELETE SET NULL;

-- Atualiza RPC para também marcar o imóvel como vendido/locado
...
BEGIN
  ...
  -- Atualiza o imóvel: marca status final, registra corretor e lead vinculado
  UPDATE imoveis
    SET status = _novo_status,
        fechado_em = now(),
        fechado_por = _lead.corretor_id,
        lead_fechamento_id = _lead_id,
        updated_at = now()
    WHERE id = _imovel_id;
